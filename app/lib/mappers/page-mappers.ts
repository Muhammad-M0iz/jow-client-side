...
import type { FormFieldDefinition } from '../../components/page/FormSection';
import type {
  DynamicComponentChild,
...
type ChildPageList = NonNullable<ContentPageProps['childPages']>;
...
const isFeaturedImageSection = (section: unknown): section is FeaturedImageSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.featured-image');

const isRichTextSection = (section: unknown): section is RichTextSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.rich-text-block');

const isGetInTouchSection = (section: unknown): section is GetInTouchSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.get-in-touch-widget');

const isQuickLinksSection = (section: unknown): section is QuickLinksSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.quick-links');

const isImportantLinksSection = (section: unknown): section is ImportantLinksSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.important-links-widget');

const isFacultyWidgetSection = (section: unknown): section is FacultyWidgetSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.faculty-widget');

const isItemGridSection = (section: unknown): section is ItemGridSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.item-grid-block');

const isNewsFeedSection = (section: unknown): section is NewsFeedSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'home-page-widgets.news-feed-widget');

const isVideoSection = (section: unknown): section is VideoSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.video');

const isPhotoAlbumSection = (section: unknown): section is PhotoAlbumWidgetSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.photo-album-widget');

const isDownloadSection = (section: unknown): section is DownloadSection =>
  Boolean(section && typeof section === 'object' && (section as { __component?: string }).__component === 'page-sections.downloads');
...
const mapFeaturedImage = (section?: FeaturedImageSection | null): ContentPageProps['featuredImage'] => {
  ...
  const media = extractMedia(section.image);
  return {
    id: section.id,
    url: resolveMediaUrl(media?.url ?? null),
...
const mapChildPages = (children?: DynamicComponentChild[]): ChildPageList => {
...
): { items: ChildPageList; heading: string | null } => {
  if (!sections.length) return { items: [], heading: null };
  const items = sections.flatMap((section) => {
    const entries = Array.isArray(section.items) ? section.items : [];
    return entries.map((item, index) => {
      const entry = item as ItemGridEntry;
      const { href, isExternal } = normalizeItemLink(entry.item_link ?? null);
      return {
        id: entry.id ?? `item-grid-${index}`,
        title: entry.item_title ?? null,
        description: entry.item_subtitle ?? null,
        href,
        isExternal,
      };
    });
  ...
  const events = sections.flatMap((section, sectionIndex) => {
    ...
    return entries.map((event, eventIndex) => {
      ...
      return {
        ...
        imageUrl: resolveMediaUrl(media?.url ?? null),
    ...
    .map<FormSectionResult | null>((form, formIndex) => {
      const fields = Array.isArray(form.fields)
        ? form.fields
            ...
            .filter((candidate): candidate is FormFieldDefinition => Boolean(candidate))
        : [];
...
const buildEventDetailSection = (eventsWidget: EventsCalendarData | null): ContentPageProps['eventDetailSection'] => {
  if (!eventsWidget) {
    return null;
  }
...
export type ContentPageMapperResult = {
  resolvedLocale: LocaleKey;
  props: ContentPageProps;
  seo: Metadata;
};

export const mapContentPage = (page: PageEntry, requestedLocale: LocaleKey): ContentPageMapperResult => {
  const resolvedLocale = normalizeLocale(page.locale ?? requestedLocale);

  const sections = Array.isArray(page.content_sections) ? page.content_sections : [];
  ...
  const navigationChildPages = mapChildPages(page.children);
...
export const buildContentPageMetadata = (page: PageEntry): Metadata => ({
  title: page.title ?? page.seo_title ?? 'Jamia Page',
  description: page.seo_description ?? undefined,
});
